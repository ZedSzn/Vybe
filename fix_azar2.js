...
console.log('GRID_USERS updated:', c.includes('Sofia') ? 'OK' : 'FAIL');

// 2b. Add showGenderPop state (after showAdvanced)
if (!c.includes('showGenderPop')) {
  c = c.replace(
    "const [showAdvanced,    setShowAdvanced]    = useState(false)",
    "const [showAdvanced,    setShowAdvanced]    = useState(false)\n  const [showGenderPop,   setShowGenderPop]   = useState(false)"
  );
  console.log('showGenderPop state added:', c.includes('showGenderPop') ? 'OK' : 'FAIL');
} else {
  console.log('showGenderPop already present');
}

// 2c. Add close-on-outside-click effect for gender popup (before countryDrop useEffect)
if (!c.includes('showGenderPop') || c.includes('if (!showGenderPop)')) {
  console.log('Gender pop effect already present or skipping');
} else {
  ...
  const countryEffectMarker = '  useEffect(() => {';
  const countryEffectIdx = c.indexOf('showCountryDrop && countryBtnRef.current');
  if (countryEffectIdx >= 0) {
    const insertBefore = c.lastIndexOf(countryEffectMarker, countryEffectIdx);
    c = c.slice(0, insertBefore) + genderPopEffect + c.slice(insertBefore);
    console.log('Gender pop effect inserted: OK');
  } else {
    console.log('Could not find countryDrop effect');
  }
}

// 2d. Remove announcement banner
const earnBannerStart = c.indexOf('EARN BANNER');
const mobileLayoutStart = c.indexOf('MOBILE LAYOUT');
if (earnBannerStart >= 0 && mobileLayoutStart >= 0) {
  const cutStart = c.lastIndexOf('\n', earnBannerStart - 1);
  const cutEnd = c.lastIndexOf('\n', mobileLayoutStart - 1);
  c = c.slice(0, cutStart) + '\n' + c.slice(cutEnd + 1);
  console.log('Banner removed: OK');
} else {
  console.log('Banner markers not found (may already be removed)');
}

// 2e. Replace desktop section — use simple text markers without Unicode box chars
const desktopMarkerText = 'DESKTOP LAYOUT';
const howItWorksMarkerText = 'HOW IT WORKS';

const desktopIdx = c.indexOf(desktopMarkerText);
const howItWorksIdx = c.indexOf(howItWorksMarkerText);

if (desktopIdx < 0 || howItWorksIdx < 0) {
  console.error('Could not find section markers! desktop:', desktopIdx, 'howitworks:', howItWorksIdx);
  process.exit(1);
}

// walk back to find the opening { of the JSX comment {/*
const openBrace = c.lastIndexOf('{', desktopIdx);
// walk back one more newline to get before the indent
const lineStart = c.lastIndexOf('\n', openBrace - 1);

// walk back to find the opening { of the HOW IT WORKS comment
const howOpenBrace = c.lastIndexOf('{', howItWorksIdx);
...
c = c.slice(0, lineStart + 1) + newDesktopSection + c.slice(howLineStart + 1);

fs.writeFileSync(mainFile, c, 'utf8');
console.log('Desktop section replaced: OK');
console.log('Done!');
